...
describe('PqsManagerFactory', () => {
  ...
  it('caches the PQS manager and disposes it once on shutdown', async () => {
    ...
    const { PqsManagerFactory } = await import('../../src/pqs/pqs-manager.factory');
    const factory = new PqsManagerFactory(async () => ({
    ...
    try {
      expect(await factory.getPqsQuery(node)).toBe(await factory.getPqsQuery(node));
  ...
  it('executes compatibility raw queries through the SDK query client', async () => {
    const queryRaw = jest.fn<
      (sql: string, values: readonly unknown[]) => Promise<readonly { id: string }[]>
    ...
    try {
      await expect(
        (await factory.getRawExecutor(node)).query('select id from things where id = $1', ['row-1']),
      ).resolves.toEqual({ rows: [{ id: 'row-1' }] });
      expect(queryRaw).toHaveBeenCalledWith('select id from things where id = $1', ['row-1']);
    } finally {
      if (originalUrl === undefined) {
        delete process.env.PQS_TEST_URL;
      } else {
